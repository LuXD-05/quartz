...
##### OS level virtualization (virtualizza copie dell’OS dell’host sullo stesso hw)
...
- Social engineering: è la capacità di spacciarsi per ciò che non si è, convincendo e manovrando gli altri per estrapolare loro informazioni riservate (per contrastarla servono precauzioni: non dare pw, chiedere tessere e id...).
...
- I dati sono salvati in server farm spesso non nello stesso paese dell'utente e in questi casi si ha meno certezza del rispetto della privacy da parte del provider,
- Con i collegamenti wireless il rischio di sicurezza aumenta per la pirateria oltre a rendere difficile il raggiungimento di soluzioni giuridiche in caso di atti illegali,
...
- Quelle pubbliche (*kpb*), ovvero chiavi di cifratura che il destinatario ricava e condivide. Il destinatario invia la sua al mittente in modo che questo critti il messaggio con essa, rendendolo decrittabile solo con la *kpr* del destinatario. 
...
Poiché gli algoritmi a chiave asimmetrica sono molto complessi e onerosi, alcune applicazioni li usano per scambiarsi solo una chiave segreta da usare per la sessione (chiave di sessione, che cambia ad ogni sessione).

( FIRMA DIGITALE, CERTIFICATI DIGITALI + CA e BLOCKCHAIN? dal libro, integrare)
...
### Firme digitali
Queste funzionalità però non garantiscono del tutto l'affidabilità dei documenti perché bisogna sapere chi ne è l'autore, garantirne l'integrità e impedirne il ripudio.
...
- "Virtuali" perché i collegamenti sono logici (non dedicati/fisici) sopra un'infrastruttura di rete pubblica (di solito internet, ma spesso aziende si appoggiano a reti geografiche fornite da un operatore con specifici protocolli).
...
#### VPN IPsec
(O Internet Protocol Security), implementa la crittografia a livello rete (quindi fornendo sicurezza al protocollo IP) e permette di creare un percorso sicuro tra 2 host comunicanti per mezzo di una VPN in una rete non sicura.
...
(foto?)
###### Tunnel mode
Questa è usata per i collegamenti VPN tra reti private che accedono a router pubblici (*gateway-to-gateway*). Viene creato un tunnel per il transito sicuro dei pacchetti IP, che vengono completamente cifrati e incapsulati in un altro pacchetto, nel cui header sono contenuti gli IP mittente e destinatario dei 2 router che sono gli estremi del tunnel. (Usando ESP si impedisce di conoscere la sorgente del pacchetto, ottimo grado di riservatezza).
(foto?)
...
4) Chiusura dell'handshake: Il serve risponde al "*change cipher spec*" con lo stesso messaggio, poi il client chiude mandando un "*client finished message*" e stessa cosa fa il server con un "*server finished message*".
###### SSL Record Protocol
