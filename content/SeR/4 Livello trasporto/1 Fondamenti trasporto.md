...
Il livello trasporto esegue una consegna **process-to-process** perché fornisce servizi ai **processi applicativi**. I protocolli di livello trasporto sono implementati **negli host** (non nei router).
...
Il livello trasporto deve quindi identificare le **applicazioni** a cui consegnare i pacchetti; ciò è fatto con il **n° di porta**, un **identificatore unico per ogni processo in un host**.
...
La comunicazione è **sempre iniziata dal client** (verso il server) che **genera dinamicamente un n° di porta sorgente univoco** per ogni conversazione (così sono possibili + di 1 in contemporanea, tipo per mandarne + a 1 un web server). (Porta sorgente e destinazione nel pacchetto livello trasporto poi incapsulati in IP avente IP sorgente e destinazione)
...
È la combinazione tra **IP** e **n° di porta** in formato \[**IP**]:\[**port**] sia **sorgente** sia **destinazione**. Identifica in processo su un host. Il socket ha l’IP dell’host e la porta generata (a cazzo) mentre a destinazione la porta è specifica. La corrispondenza tra socket sorgente e socket destinatario è detta ***socket pair***, e discrimina una conversazione.
