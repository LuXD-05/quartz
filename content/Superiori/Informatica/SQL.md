...
public: true
edited_seconds: 3370
modified_at: 22/04/2024 09:59:37
---
...
### Tipi di richieste
##### Identificazione
Per identificare le richieste bisogna suddividere il testo in parti atomiche, per esempio: 
###### Esempio 1
"Trovare il nome e il costo dei prodotti che hanno un prezzo superiore alla media" diventa:
- "Trovare il nome e il costo" = `SELECT nome, costo`
- "Dei prodotti" = `FROM prodotti`
- "Che hanno un prezzo > di" = `WHERE costo >` 
- "Media" = `AVG(costo)`
  `AVG(costo)` non si può inserire direttamente nella query dato che non è richiesta una `GROUP BY`, quindi si userà una subquery così: `(SELECT AVG(costo) FROM prodotti)`.
###### Esempio 2
"Elencare per ogni cliente la spesa media superiore alla media della spesa di tutti i clienti" (supponendo che le tabelle siano "clienti" --> "acquista" --> "pezzi" --> "catalogo") si divide in:
1) Separiamo la query interna dalla esterna:
   - Main: "elencare per ogni cliente la spesa media" = `SELECT spesa FROM ... GROUP BY c.id HAVING AVG(spesa)` 
   - Inner: "la media della spesa di tutti i clienti" = `SELECT AVG(spesa) FROM ...`
2) da "spesa" (contenuta tramite costo in "catalogo") e "cliente" (da tabella) per "elencare ... spesa ... clienti" serve joinare le tabelle così: "clienti" --> "acquista" --> "pezzi" --> "catalogo".
...
```sql
SELECT c.nome, c.cognome, AVG(ca.costo) AS spesaMedia
FROM __clienti c
JOIN __acquista a ON c.id = a.cid
JOIN __pezzi p ON a.pid = p.id
JOIN __catalogo ca ON p.id = ca.pid
GROUP BY c.id
HAVING AVG(ca.costo) > (
	SELECT AVG(cac.costo) 
    FROM __catalogo cac 
    JOIN __pezzi p ON cac.pid = p.id
    JOIN __acquista a ON p.id = a.pid 
    JOIN __clienti c ON a.cid = c.id
)
```
##### Identificazione 2
Generalmente, per risolvere query scalari o condizionate su + righe, si scompone la query in 2:
- La *main query*, ovvero la selezione esterna.
- La *inner query*, ovvero la selezione con cui si vuole verificare una condizione sulla query esterna.
L'ordine di identificazione è indifferente, però questo funziona sulle ***self join*** solo se joinate <u>esplicitamente</u>.
##### Query normali
Query che non necessitano di subquery.
##### Query scalari
Ovvero query con una single-row subquery e che si confrontano con essa con un operatore adatto. Tipo:
- "Trovare il nome e il costo dei prodotti che hanno un prezzo superiore alla media"
- "Trovare il nome del pezzo con il prezzo più alto"
##### Query condizionate su + righe
Query ai cui risultati va imposta una condizione con una query che restituisce + di 1 riga (solitamente). Tipo:
###### General cases
- NOT IN nella main query
	- 
- IN nella main query
- ALL nella main query
- ANY nella main query
###### Case "not"
"Trovare gli id dei fornitori che non forniscono pezzi rossi o pezzi blu"
```sql
SELECT DISTINCT f.id
FROM __fornitori f
JOIN __catalogo ca ON f.id = ca.fid
JOIN __pezzi p ON ca.pid = p.id
WHERE f.id NOT IN (
	SELECT f.id
	FROM __fornitori f
	JOIN __catalogo ca ON f.id = ca.fid
...
```
###### Case "only"
"Trovare gli id dei fornitori che forniscono o solo pezzi rossi o pezzi verdi"
```sql
SELECT DISTINCT f.id
FROM __fornitori f
JOIN __catalogo ca ON f.id = ca.fid
...
WHERE f.id NOT IN (
	...
	JOIN __catalogo ca ON f.id = ca.fid
	JOIN __pezzi p ON ca.pid = p.id
	WHERE p.colore != 'rosso' AND p.colore != 'verde'
)
```
###### Case "at least"
"Trovare gli id dei fornitori che forniscono almeno 1 pezzo verde"
```sql
SELECT DISTINCT f.id
FROM __fornitori f
JOIN __catalogo ca ON f.id = ca.fid
...
```
###### Case "at most"
"Trovare gli id dei fornitori che forniscono al massimo 1 pezzo verde"
```sql
SELECT DISTINCT f.id
FROM __fornitori f
JOIN __catalogo ca ON f.id = ca.fid
...
1) Trovare il nome e il costo dei prodotti che hanno un prezzo superiore alla media.  
2) Trovare il nome del pezzo con il prezzo più alto  
3) Trovare gli id dei fornitori che forniscono o solo pezzi rossi o pezzi verdi
4) Trovare i fornitori che vendono solo divani  
5) Trovare i fornitori che non hanno articoli in catalogo  
6) Trovare gli id dei fornitori che forniscono solo pezzi verdi  
7) Estrarre i nomi dei clienti che sono anche cognomi  
8) Elencare per ogni cliente la spesa media superiore alla media della spesa di tutti i clienti

1) Elencare tutti gli impiegati che hanno lo stessa mansione dell'impiegato di nome Mario Bianchi  
2) Elencare tutti gli impiegati che hanno uno stipendio superiore alla media degli stipendi di tutti gli impiegati  
3) Elencare l'impiegato che ha lo stipendio più alto  
4) Trovare i dipendenti dei dipartimenti con sede a Milano  
5) Trovare i dipendenti dei dipartimenti con sede diversa da Milano  
6) Estrarre il nome e cognome impiegati che non lavorano in dipartimenti situati a FIRENZE  
7) Estrarre il cognome e nome degli impiegati i cui cognomi sono anche nomi  
8) Estrarre i nomi degli impiegati che sono anche cognomi  
9) Estrarre i nomi degli impiegati che non sono cognomi per qualche impiegato  
10) Estrarre i dipartimenti in cui non lavorano persone di cognome Rossi  
11) Estrarre gli impiegati che lavorano in dipartimenti situati a Firenze  
12) Determinare i dipartimenti che abbiano almeno un impiegato che guadagna più di 1k  
13) Determinare i dipartimenti tali che tutti gli impiegati di tali dipartimenti guadagnino più di 1600  
14) Elencare per ogni dipartimento il valore medio degli stipendi superiori alla media degli stipendi del dipartimento  
15) Trovare gl impiegati che hanno stesso nome e cognome ma mansione divers