...
Trasformando *key* in indirizzo, la funzione di hash è usata per <u>trovare la posizione di un dato nella tabella</u> e deve rispettare certe caratteristiche:
...
Si utilizza una tabella (array) $T$ di dimensione fissa $n$ e in in ogni posizione di $T$ vi è un riferimento al 1° nodo di una lista concatenata $L_{i}$. Data una chiave $c$, applicandovi la funzione hash $H$ con $H(c)$, si ottiene un **numero** associato alla chiave che serve per ottenere l'**indice** $i$ che indica in che lista della tabella si trova $c$ (per ora diciamo che $H(c) = i$, ovvero ritorna diretto l'indice $i$). Metodi: 

- `member(c)` scorre la lista in $T[H(c)]$ fino a trovare $c$,
- `insert(c)` inserisce all'inizio della lista in $T[H(c)]$,
- `delete(c)` come `member(c)` ma quando trova $c$ lo cancella.
...
> Per l'uso di liste, ogni operazione costa come farla su lista normale ${} O(L.size()) {}$; se poi si ha una stima del n° di dati attesi, si ha in media un costo di $O(1)$ scegliendo $n \approx m$.
> \[TEOREMA]...

###### Hash statico con indirizzamento aperto

Qui i dati sono inseriti direttamente in $T$ (array di $n$ elementi) che deve avere $n \geq m$ (dimensione $\geq$ dati attesi). Siccome in ogni $T[i]$ c'è un dato e non una lista, sorge il problema delle **collisioni** quando:

- `insert(c)`: ma $T[H(c)] \neq$ `null` (ovvero quando in ${} T[H(c)] {}$ c'è già un elemento),
- `member(c)` e `delete(c)`: quando $T[H(c)] = x$ e non $c$ (ovvero quando cerco/elimino $c$ in $T$ ma in $T[H(c)]$ c'è $x$ e non $c$).
...
Prossima lezione: [[10 - Attraversamento dei grafi]]
