...
Così ad ogni **partizione** corrisponde un **albero** la cui <u>radice</u> ne è l'<u>elemento rappresentativo</u>. Perciò con $A = \{1 \ldots 9\}$ e partizioni $P = \{\{1,3,{\color{yellow}{7}}\}, \{{\color{yellow}{2}}\}, \{4,5,6,7,{\color{yellow}{8}},9\}\}$ si definisce un <u>array di padri</u> (o di radici) che per ogni elemento di $A$ indica il suo più alto antenato (root), ovvero l'<u>elemento rappresentativo</u>: $padri = [7,2,7,8,8,8,7,8,8]$.
	...
	*/
	public void Union(int x, int y) {        // Unisce la partizione più piccola a quella più grande
		...
		if (size[rootX] < size[rootY]) {     // Se part x < part y
			...
			father[rootX] = rootY;           // Viceversa ma x > y --> y vede padre sostituito e aumenta la size di x
