...
###### Somma di n numeri in input

```armasm
READ 1      // Carica il prossimo valore dal nastro di input in R1
JBLANK 8    // Se non c'è un valore seguente in input --> va a 8 (WRITE 1)
READ 2      // Carica prox input in R2
LOAD 1      // Copia valore R1 in R0
ADD 2       // Fa R0 = R0 + R2
STORE 1     // R1 = R0 (carica la somma di nuovo in R1)
JUMP 2      // Torna a 2 (JBLANK 8)
WRITE 1     // Scrive valore di R1 in output
HALT
```

CCU: 

spazio = 3 registri (R0, R1, R2) = $O(1)$

tempo = 3 istruzioni fuori dal ciclo + 6 istruzioni nel ciclo (eseguite $n$ volte) = $6n + 3$ = $\Theta(n)$

CCL:

spazio = registro con valore maggiore è R1 = $\sum\limits_{i = 0}^{n} x_{i}$ (somma dei numeri da 1 a $n$) però con logaritmo = $\log_{2}\left(\sum\limits_{i = 0}^{n} x_{i}\right) + 1$ = $O(\log n)$

tempo = boh troppo difficile 💀
