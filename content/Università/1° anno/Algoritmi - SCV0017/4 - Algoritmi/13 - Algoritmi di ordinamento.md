	...
	public static void sort(Comparable[] arr) {
		...
		for (int i = 0; i < n; i++) {          // Scorre ogni elemento dal 1° a ultimo
			...
			for (int j = n-1; j > i; j--) {    // (Per ogni elemento) ri-scorre da ultimo ad arr[i]
				...
				if (arr[j] < arr[j-1])) {      // o if (less(arr[j], arr[j-1]) per i Comparable
					exchange(arr, j, j-1);
					ex = true;
				}

				if (!ex) break;                // Se non scambia (sx < dx) --> passa al ciclo successivo
...
**Ottimale**: complessità $\Theta(n \log n)$ nel caso peggiore.
