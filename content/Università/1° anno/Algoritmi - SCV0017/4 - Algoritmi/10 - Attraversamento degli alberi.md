...
public void Preorder(Node n) {
	...
	while (!s.isEmpty()) {
		...
		if (n.left != null)
			s.push(n.left);     // Se c'è un figlio a destra --> push in stack
		if (n.right != null)
			s.push(n.right);    // Se c'è un figlio a sinistra --> push in stack
	}
}
```
...
Il ***levelorder*** invece è identico all'algoritmo [[11 - Attraversamento dei grafi#Graph Traversal Algorithms|BFS]] che è applicabile anche a certi tipi di grafi, seleziona i nodi ad ogni livello da sinistra a destra o viceversa.
