...
> $$|A \cap B| \leq |A| \;\land\; |A \cap B| \leq |B|$$
...
Dato $A = \{a,b,c\}$ e $A^{(4)} =$ insieme delle parole di 4 lettere fatte con le lettere di $A$:
...
- $|P(A \times P(B))| = 2^{|A| \times 2^{|B|}} = 2^{2 \times 2^{2}} = 2^{8} = 256$
- $|P(A \times (A \cap B))| = 2^{|A| \times 0} = 2^{0} = 1$
