...
Si verifica se si trova nella tabella una riga ed una colonna (di uno stesso indice) in cui tutti i valori corrispondono agli indici stessi ([[#1) Esercizio generico|esempio]]).
...
> [!info] Quindi
> La simmetrizzabilità si riferisce all'<u>indice di riga/colonna</u> (vedi [[#Esempio 8]]).
...
- **Commutativa**? Si, perché $x*y = y*x$,
- **Associativa**? Si, perché ${} (x*y)*z = x*(y*z) = x*y*z$,
- **Elemento neutro**? $[1]_{4}$ siccome $x * [1]_{4} = [1]_{4} \;\land\;[1]_{4} * x = [1]_{4}$,
...
Perciò ($\mathbb{Z},*$) <u>non è un gruppo</u>.
