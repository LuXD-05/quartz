...
Il **principio di induzione** è una tecnica di dimostrazione di tesi matematiche che consente di dimostrarne la validità dalla verifica di 2 condizioni; tuttavia, vale <u>solo</u> per gli enunciati formulati in funzione di $\mathbb{N}$.
...
2) Per il **passo** di induzione invece: 
   ...
   1) Sostituire $(n+1)$ ad $n$ nella formula di $P$ (sopra la sommatoria/produttoria e nel risultato) ottenendo $P(n+1)$,
   2) Riscrivere $P(n+1)$ come $P(n) * (n+1)$ in base all'operazione, ovvero sostituendo $n+1$ a $k$ (tipo: "$\displaystyle\sum\limits^{n+1}_{k=1} (\tfrac{k}{2}) \rightarrow \sum\limits^{n}_{k=1} (\tfrac{k}{2}) + \tfrac{n+1}{2}$"),
   3) Sostituire a $P(n)$ il valore (risultato) della sua formula originale, se la formula ottenuta coincide con quella del (sub)step 1 $P(n+1)$, allora il **passo** è verificato (quindi come anche $P$ stessa per ipotesi di induzione).
