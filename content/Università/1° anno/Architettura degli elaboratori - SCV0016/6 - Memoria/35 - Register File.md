...
Ciò che è appena stato mostrato è il ***datapath***, ovvero un'insieme di unità tipo **ALU** e **registri** necessari per eseguire le istruzioni della CPU.

Si dice poi ***ciclo*** di *datapath* il passaggio di 2 operandi attraverso la ALU e la memorizzazione del risultato in un registro.
...
5) ($CK = 1$) Gli $m$ gruppi di **[[21 - Multiplexer#Multiplexer|MUX]]** (qui $m = 2$) restituiscono in output le $m$ *word* che i registri agli indici $o_{1}$ e $o_{2}$ stanno restituendo in output (ricordiamo che i [[21 - Multiplexer#Multiplexer|MUX]] implementano un [[20 - Decoder#Decoder|DEC]], per questo non ce n'è bisogno per decodificare $o_{1}$ e $o_{2}$).
