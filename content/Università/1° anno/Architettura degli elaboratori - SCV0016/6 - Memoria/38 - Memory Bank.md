...
###### Memory bank con più word più lunghe

Se volessimo invece implementare un *bank* sia con più *word* sia con *word* più lunghe, allora si combinano i precedenti in in questo modo (per uno da 64x128 bit):

![](https://i.imgur.com/oKtdq4N.png)

Per *word* più lunghe si collegano 2 chip come fatto [[#Memory bank con *word* più lunghe|qui]], mentre per aumentare il n° di *word* disponibili (sempre più lunghe dei normali 32 bit nei chip) si duplica l'infrastruttura appena implementata (necessari solo 2 $CS$ e $OE$).

##### Organizzazione a matrice

Le memorie che molti immaginano come vettori **monodimensionali** di *word*, soprattutto a capacità più elevate, hanno una struttura fisica **bidimensionale** (per ridurre la lunghezza dei collegamenti interni):

![](https://i.imgur.com/FCrYAu6.png)

Nota che i chip raggruppati in rosso sono uniti per allungare le *word* (da 8 a 16 bit), in più, a questi 2 gruppi logici di chip, arrivano $CS$ e $OE$ diversi, il che indica che i 2 <u>gruppi</u> sono a loro volta agglomerati per ottenere più *word* (da 1M a 2M).

Si ottiene quindi un *bank* da 2Mx16 bit:

![](https://i.imgur.com/eW0Ke9N.png)
...
Prossima lezione: [[39 - CPU a ciclo singolo]]
