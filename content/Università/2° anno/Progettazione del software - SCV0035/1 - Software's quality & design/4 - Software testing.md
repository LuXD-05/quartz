...
*Testing* e *analysis* sono entrambi attuati in un ciclo di fasi: 

1) **Planning & analysis**: necessario per acquisire informazioni e ridurre rischi,
2) **Test design**: si progettano i test creando la *test data* e i *test cases*,
3) **Test execution**: si eseguono i test e si verifica che gli output del programma sono uguali a quelli scritti nei *test cases*,
4) **Defect management**: si correggono errori e bug.

> [!important] Debugging
> Nota come non è presente il ***debugging*** tra le fasi; questo perché il *debugging* è un'attività svolta dai <u>developer</u> e non dai <u>tester</u> (anche se a volte sono la stessa figura) e serve principalmente a <u>trovare bug</u> per correggerli nel codice, mentre col *testing* l'obiettivo è <u>stimare la qualità del software</u> e ad <u>aumentarne l'affidabilità</u>.
...
La **sensibilità** (o meglio <u>coerenza</u>) significa che un test è efficace <u>se segnala sempre un errore, non solo qualche volta</u> (esempio: un test del codice `double n = 1/input;` non è affidabile in quanto darà sempre corretto tranne quando l'input è 0; oppure un test che segnala errore su una macchina e non su un'altra...).

###### Redundancy

Essere **ridondanti** durante l'analisi aiuta a scovare errori prima e meglio (usare tipi di variabile statici e non dinamici, verificare i requisiti prima del software finale...).

###### Partitioning

Vengono individuate **partizioni** nel codice da testare al fine di suddividere gli input per gestirli meglio (più semplice creare input per 1 procedura che per l'intero software).

###### Restriction

Si possono gestire problemi complessi **restringendoli** grazie a regole e vincoli (variabile di tipo dinamico da errore a runtime, con tipo statico da errore più specifico).

###### Feedback

L'**esperienza** costruita negli anni è fondamentale, siccome serve per costruire delle *checklist* di problemi comuni già riscontrati in precedenza per scrivere test migliori.
