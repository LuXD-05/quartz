...
Nell'esempio la `ShapeFactory` è il *creator* che, in base a certi <u>parametri</u>, determina la `Shape` da creare e la sceglie tra quelle che ereditano da `Shape`, così da dover importare solo la *factory* nei moduli e non tutte le `Shape` ogni volta. In codice:
...
// ... Classi Circle / Square / Rectangle che implementano Shape ed il metodo draw() ...

public class ShapeFactory {
	// Metodo che ritorna l'istanza della Shape richiesta in base al parametro "type"
...
La ***abstract factory*** invece si usa quando si hanno delle <u>famiglie di oggetti</u> per ognuna delle quali si vuole avere una factory; tali factory permetteranno la creazione solo di oggetti della famiglia specifica per cui sono definite, così da poter importare solo esse invece che tutta la famiglia. 

Le factory condividono solo un'interfaccia "factory" astratta che definisce i loro metodi e per cambiare famiglia basta istanziare un'altra factory.

###### Esempio abstract factory

Supponiamo di star trattando forme 2D e i corrispondenti oggetti 3D, questo si può risolvere con un'*abstract factory* che definisce le forme per la factory concreta 2D e i relativi oggetti per la factory concreta 3D:

```java
// La ShapeFactory diventa astratta (ora interfaccia con metodi)
public interface ShapeFactory {
    Shape createCircle();
    Shape createSquare();
}
// ... Le classi che ereditano da Shape sono le stesse + i corrispettivi oggetti 3D ...
// Factory concreta 2D
...
public class Shape3DFactory implements ShapeFactory {
    public Shape createCircle() { return new Sphere(); }
    public Shape createSquare() { return new Cube(); }
}
```

(Un esempio un po' più chiaro potrebbe coinvolgere una *abstract factory* per la creazione di componenti grafici da renderizzare su Windows e su MacOS, dove le factory concrete sarebbero `WinComponentFactory` e `MacComponentFactory`).
...
Il ***composite*** pattern si usa per la gestione di oggetti in strutture gerarchiche ad albero, i cui componenti sono ***leaves*** (foglie, oggetti singoli) e ***composites*** (contenitori di foglie) dello stesso tipo `Component`. La caratteristica principale è che quando si chiama un metodo in una foglia lo si esegue, mentre se lo si fa in un *composite*, esso inoltra la chiamata a tutte le sue foglie.

Il vantaggio del *composite pattern* è che rende semplice l'aggiunta di nuovi tipi di componenti, tuttavia ciò può rendere il progetto troppo generico in quanto diventa poi difficile imporre restrizioni ad essi.

###### Esempio composite

Supponiamo di voler progettare un *file system* con componenti `File` (foglia) e `Folder` (*composite*):
...
public class Folder implements FileSystemComponent {
    ...
    public void ls() {
        // ...
        for (FileSystemComponent c : children) { c.ls(); } // ricorsivo
    }
}
```

In questo modo, facendo `ls()` su una `Folder` la chiamata al metodo verrà propagata a tutti i suoi figli siccome anch'essi sono `FileSystemComponent`.

#### Façade

Quando si ha un sistema complesso (tante classi, dipendenze, ordini di call precisi) col quale utenti interagiscono, per evitare che essi <u>conoscano i dettagli del sistema</u> o lo sfruttino in modi che non dovrebbero, gli si fornisce una ***façade***: un'interfaccia contenente solo certi metodi disponibili e definiti preventivamente con le operazioni in ordine preciso.

###### Esempio façade

Un sistema di gestione degli ordini deve prevedere tante azioni atomiche e magari non si vuole che l'utente le usi singolarmente o le conosca:

```java
Inventory.check(product);
Payment.authorize(card);
Shipping.reserve(product);
Notification.send();
Invoice.create();
```

Usando invece una *façade* si avrebbe solo un metodo `placeOrder()`:

```java
public class OrderFacade {
    public void placeOrder(Product p, Card c) {
        Inventory.check(p);
        Payment.authorize(c);
        Shipping.reserve(p);
        Invoice.create(p);
...
L'***observer** pattern* è molto usato nei sistemi ad eventi e fa uso (per l'appunto) di oggetti detti ***observer***, i quali "osservano" eventi e/o cambiamenti di stato di un ***subject***. La caratteristica fondamentale è che il *subject* contiene dei meccanismi che permettono a diversi *observer* di "iscriversi" a certi eventi lanciati da esso cosicché (gli *observer*) vengano informati quando il *subject* li lancia.

Questo pattern è molto utile e usato in sistemi con interfacce grafiche e in librerie esterne che devono notificare cambiamenti di stato agli oggetti di un programma.

###### Esempio observer

Facciamo l'esempio della gestione di uno stock dove dei trader vogliono essere notificati dei cambiamenti allo stock:
...
public class Trader implements Observer {
	// ... Attributi e ctor ...
	// Metodo che osserva lo stock
    public void update(double price) { System.out.println("Prezzo aggiornato a: " + price); }
}

// Subject
public class Stock {
    private double price;
    // Lista di observer associati
    private List<Observer> observers = new ArrayList<>();
    
    // Metodi per associare / disassociare un observer
    public void attach(Observer o) { observers.add(o); }
    public void detach(Observer o) { observers.remove(o); }
    
	// Metodo osservato (cambio di prezzo)
    public void setPrice(double price) {
        this.price = price;
        notifyObservers();
    }
    
	// Metodo che notifica gli observer
...
Esistono tanti altri tipi di pattern oltre ai *design pattern*, quali:

- ***Architectural patterns***: utili per strutturare il sistema in sottosistemi,
- ***Idioms***: descrivono (a basso livello) come implementare certi aspetti di certi componenti usando certi linguaggi di programmazione.

##### MVC

L'**MVC** (*Model-View-Controller*) è un pattern architetturale che struttura applicazioni in sistemi a strati (dove ognuno dei quali comunica solo con gli adiacenti) con 3 tipi di componenti:

- ***Model***: i dati dell'applicazione (classi),
- ***View***: l'interfaccia utente che permette interazioni dall'esterno (anche con altri sistemi),
- ***Controller***: gestisce la comunicazione tra view e models.
