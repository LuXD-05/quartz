...
c'è una piscina con 100 token, gli utenti sono di 3 tipi: 1) usa 1 token, 2) usa 2, 3) usa 3.

se ci sono n token disponibili un utente n entra in piscina, mentre se non ce ne sono abbastanza aspetta
...
semaphore s2 = 0;
semaphore s3 = 0;
int wt1 = 0, wt2 = 0, wt3 = 0, wr1 = 0, wr2 = 0, wr3 = 0;

// T1
while (true) {
	wait(mutex);
	if (wr1 > 0 || wr2 > 0 || wr3 > 0) {
		wt1++;
		signal(mutex);
		wait(s1);
		wait(mutex);
	...
	wait(mutex);
	wr1--;
	if (wt2 > 0 && (wr1 > 0 || wr2 > 0 || wr3 > 0)) {
		wt2--;
		signal(s2);
	} else if (wt3 > 0 && (wr1 > 0 || wr2 > 0)) {
		wt3--;
		signal(s3);
	} else if (wt1 > 0 && (wr1 > 0 || wr2 > 0 || wr3 > 0)) {
		wt1--;
		signal(s1);
	}
	signal(mutex);
...
while (true) {
	...
	if (w4 > 0 && posti >= 4) {
		w4--;
		signal(s4);
	} else if (w3 > 0 && posti >= 3) {
		w3--;
		signal(s3);
	} else if (w2 > 0 && posti >= 2) {
		w2--;
		signal(s2);
	}
