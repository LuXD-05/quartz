...
L’esecuzione di un programma passa attraverso tre fasi concettualmente distinte che trasformano il codice sorgente in un programma eseguibile residente in memoria.

- Traduzione (assembler/compilatore), linking (risoluzione simboli), loading (allocazione in memoria)

##### Programma sorgente, object module e binario

Il compilatore/assembler genera object module contenenti codice macchina con riferimenti simbolici; il linker risolve tali riferimenti e produce un binario eseguibile, che il loader carica in memoria rilocando gli indirizzi se necessario.

- Rilocazione per evitare overlapping tra moduli e programmi
