...
> ```SQL
> SELECT XMLELEMENT(NAME "title", title) AS "Movie Titles" FROM movie;
> ```
...
- `NULL ON EMPTY`: indica che per gli elementi vuoti, nella tupla risultante sarà inserito `NULL`.
...
- Il ***column pattern***: che fa uso della *keyword* `COLUMNS` dopo la quale si specifica una colonna per riga con la sintassi: `"[col]" [type] PATH '[exp]',` (`col` e `type` sono autoesplicativi dall'esempio, mentre `PATH` indica che l'espressione XQuery `exp` seguente determina il valore di ogni riga nella colonna).
