...
Oltre alla keyword `FINAL` (usata per indicare se un tipo può avere sottotipi o meno), esiste anche la keyword `INSTANTIABLE` (di default) che indica se il tipo/classe è istanziabile o meno (un tipo `NOT INSTANTIABLE` e `FINAL` non è utilizzabile, siccome sarebbe astratto e non raffinabile al contempo).
...
Quando si esegue una query su una supertabella, al si sta eseguendo anche su tutte le sue sottotabelle, per esempio:
