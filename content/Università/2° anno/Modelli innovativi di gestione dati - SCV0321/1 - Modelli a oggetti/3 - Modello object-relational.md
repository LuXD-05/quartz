...
```sql
CREATE TABLE Person(
	name NameType, -- Riferito al NameType precedente
	-- ...
	birth DATE
);
```

Si va quindi a definire una <u>struttura nidificata</u> delle tabelle (attributi che hanno sotto-attributi); per cui il modello relazionale diventa <u>modello relazionale nidificato</u>, rendendo non più necessaria l'eliminazione degli attributi composti nella fase di progettazione logica.
...
CREATE TYPE PersonType AS (
	...
	address Address,
	birth DATE
) METHOD age_on_date(on_date DATE) RETURNS INTERVAL YEAR; -- Firma metodo
...
CREATE INSTANCE METHOD age_on_date(on_date DATE)
	...
	RETURN on_date - SELF.birth    -- Keyword "SELF": usata x riferirsi all'istanza del tipo a cui il metodo...
```
