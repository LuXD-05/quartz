...
> Un ***graph pattern*** è un'espressione che identifica certi *edge-labelled graphs* facendo uso di **variabili** per le etichette. Un esempio che identifica delle triple (***basic graph pattern***) è: `x insegna MIGD`; da qui il processo di ***matching*** <u>controlla tutte le triple esistenti</u> e <u>seleziona</u> tutte <u>quelle che combaciano</u> con le risorse del *pattern*.
