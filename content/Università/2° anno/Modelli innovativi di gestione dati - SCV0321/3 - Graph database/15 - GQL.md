...
> ```graphql
> MATCH TRAIL (x1 :Person) -[:acts_in]-> (m:Movie WHERE title == "Unforgiven") <-[:acts_in]- (x2 :Person)
> RETURN x1.name, x2.name, m.title
> ```
...
> ```graphql
> USE Fraud {
> 	MATCH (x) -[z:transfer WHERE z.amount > 100]-> (y WHERE y.Blocked == true)
