...
An **attacker send more requests to a pc than the ones it can handle**. Then, eventually, the <u>attackers connects on a service port to make forged packets enter the user's network</u> (more on [[DDoS]]).
